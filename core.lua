...
function GGRL:ZONE_CHANGED_NEW_AREA()
  local _, instanceType, _, _, _, _, _, id = GetInstanceInfo()
...
function GGRL:StartEncounterTimer(encounterID)
  GGRL:Print("Timer started")
  GGRL:RegisterEvent("COMBAT_LOG_EVENT_UNFILTERED")
  GGRL.currentBoss = encounterID
  GGRL.currentBossPhase = 1
  GGRL.currentBossTimes = GetEventTimes(GGRL.Bosses[encounterID][1])
  GGRL.combatTimer = GGRL:ScheduleRepeatingTimer("TimerTick", 1)
end

function GGRL:StopEncounterTimer()
  GGRL:UnregisterEvent("COMBAT_LOG_EVENT_UNFILTERED")
  GGRL:CancelTimer(GGRL.combatTimer)
...
GGRL:RegisterEvent("ZONE_CHANGED_NEW_AREA")
